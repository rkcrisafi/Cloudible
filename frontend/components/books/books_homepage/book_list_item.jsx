...
const BookIndexItem = ({ book, idx }) => {
  ...
  return (
    <li>
      { !Array.isArray(book) ? (
       <div data-idx={book.idx} className="lbook-index-item homepage-each-element">
        ...
        (<div className="lbook-small-book-index">
          {book.map((b, i) => (
            <Link data-idx={b.idx} key={i} to={`/books/${b.book.id}`}>
              <img data-idx={b.idx} src={b.book.imageUrl} width="140" height="140" className="lindex-book-small-image-link homepage-each-element"/>
            </Link>))}
        </div>)
          }
      </li>
